      ...
      {showBilling && (
        <div className="billing-overlay" role="dialog" aria-modal="true" aria-label="Plans and billing">
          <section className="billing-panel">
            ...
            <div className="billing-current"><div><span>CURRENT PLAN</span><strong>{billingLoading ? "Checking subscription…" : billingStatus?.is_premium ? "Premium" : "No active subscription"}</strong>{!billingLoading && billingStatus?.is_premium && billingStatus.current_period_end && <small className="billing-period-end">{billingStatus.cancel_at_period_end ? "Premium ends" : "Current access ends"} <time dateTime={billingStatus.current_period_end}>{formatBillingDate(billingStatus.current_period_end)}</time></small>}</div><button disabled={billingLoading} onClick={() => refreshBillingStatus().catch((err) => setBillingError(err.message))}>{billingLoading ? "Checking…" : "Refresh"}</button></div>
