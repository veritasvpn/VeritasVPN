...
WORK = ROOT / "video" / "incognito-vs-vpn"
SCENES = WORK / "scenes"
OUTPUT = WORK / "output"
SEGMENTS = WORK / "segments"
TRANSCRIPT_JSON = Path("/private/tmp/incognito-misconception-transcript.json")
AUDIO = Path("/Users/juanpablogarcia/Downloads/IncognitoMisconception.wav")

SCENE_CUTS = [0.0, 8.08, 12.86, 24.08, 34.32, 37.48, 58.68, 69.24, 80.350]
...
def render_caption_track(
    ...
    entry_index = 0
    for frame in range(math.ceil(80.350 * fps)):
        time = frame / fps
...
def main() -> None:
    ...
    for index, group in enumerate(groups, start=1):
        start = group[0]["start"]
        next_start = groups[index][0]["start"] if index < len(groups) else 80.350
        end = min(80.350, group[-1]["end"] + 0.08, next_start - 0.02)
    ...
    silent_video = OUTPUT / "incognito-vs-vpn-silent.mp4"
    ...
    final_video = OUTPUT / "Incognito-Mode-vs-VPN.mp4"
