...
WORK = ROOT / "video" / "what-is-an-ip-address"
SCENES = WORK / "scenes"
OUTPUT = WORK / "output"
SEGMENTS = WORK / "segments"
TRANSCRIPT_JSON = Path("/private/tmp/what-is-an-ip-address-transcript.json")
AUDIO = Path("/Users/juanpablogarcia/Downloads/--what-is-an-ip-address-----ev.wav")

DURATION = 84.95
# Public-IP scene returns after the private-IP explanation because the narration
# then describes what a public IP may reveal.
SCENE_SEQUENCE = [
    (1, 0.0, 7.94),
    (2, 7.94, 18.68),
    (3, 18.68, 26.80),
...
def main() -> None:
    ...
    # The source recording audibly contains formatting artifacts immediately
    # before “public” and “private”. Keep captions on the intended narration.
    words = [
        word
        for word in words
        if not (
            (
                37.0 <= word["start"] < 37.92
                and word["word"].strip().lower() in {"asterisk", "is", "to", "risk"}
            )
            or (
                46.0 <= word["start"] < 46.72
                and word["word"].strip().lower() == "asterisk"
            )
        )
    ]
    for word in words:
        if word["word"].strip() == "A" and abs(word["start"] - 37.04) < 0.02:
            word["end"] = 37.92
        if word["word"].strip() == "A" and abs(word["start"] - 46.02) < 0.02:
            word["end"] = 46.72
    ...
    for index, group in enumerate(groups, start=1):
        ...
        text = (
            text.replace("ISB", "ISP")
            .replace("Veritas VPN", "VeritasVPN")
            .replace("Internet service provider", "Internet Service Provider")
        )
    ...
    silent_video = OUTPUT / "what-is-an-ip-address-silent.mp4"
    ...
    final_video = OUTPUT / "What-Is-an-IP-Address.mp4"
