...
Local copies of downloadable artifacts. **Production traffic** is served by Cloudflare Functions that stream from GitHub Releases (Android + Linux `v0.2.34`). These files keep the repo / Dell hostPath fallback aligned with that release.

| File | Purpose |
|------|---------|
| `veritasvpn-android.apk` | Signed Android release (`0.2.29`) — must match GitHub `v0.2.34` SHA-256 |
| `veritasvpn-linux.deb` | Linux .deb (`0.2.34`) — gitignored; keep in sync on disk for hostPath |
| `veritasvpn-linux.AppImage` | Linux AppImage (`0.2.34`) — gitignored; keep in sync on disk for hostPath |
