...
Local copies of downloadable artifacts. **Production traffic** is served by Cloudflare Functions that stream from GitHub Releases (Linux `v0.2.41`; Android APK still from `v0.2.34`). These files keep the repo / Dell hostPath fallback aligned with that release.
